 ...
 * bottom and left hand edges.  Centers title at top of window.
 */
void DrawWindow(LocalWin *win_info)
{
...
/*
 * This routine draws the title of the graph centered in the window.  It is
 * spaced down from the top by an amount specified by the constant PADDING.
 * The font must be fixed width.  The routine returns the height of the title
 * in pixels.
 */
static void DrawTitle(LocalWin *wi)
{
	if (wi->DOrder == 0) {
		wi->dev_info.xg_text(wi->dev_info.user_state, (int)(wi->dev_info.area_w*0.95) , wi->dev_info.axis_pad, PM_STR("TitleText"), T_UPPERRIGHT, T_TITLE);
	}
	else if (wi->DOrder == 1) {
		wi->dev_info.xg_text(wi->dev_info.user_state, (int)(wi->dev_info.area_w*0.95) , wi->dev_info.axis_pad, "First Derivative", T_UPPERRIGHT, T_TITLE);
	}
	else if (wi->DOrder == 2) {
		wi->dev_info.xg_text(wi->dev_info.user_state, (int)(wi->dev_info.area_w*0.95) , wi->dev_info.axis_pad, "Second Derivative", T_UPPERRIGHT, T_TITLE);
	}
}

/*
 * This routine figures out how to draw the axis labels and grid lines.  Both
 * linear and logarithmic axes are supported.  Axis labels are drawn in
 * engineering notation.  The power of the axes are labeled in the normal axis
 * labeling spots.  The routine also figures out the necessary transformation
 * information for the display of the points (it touches XOrgX, XOrgY, UsrOrgX,
 * UsrOrgY, and UnitsPerPixel).
 */
static int TransformCompute(LocalWin *wi)
{
	...
	wi->XOrgX = wi->dev_info.bdr_pad + (7 * wi->dev_info.axis_width) + wi->dev_info.bdr_pad;
	wi->XOrgY = wi->dev_info.bdr_pad + wi->dev_info.title_height + wi->dev_info.bdr_pad + wi->dev_info.axis_height + wi->dev_info.axis_height / 2 + wi->dev_info.bdr_pad;
...
/*
 * This routine draws grid line labels in engineering notation, the grid lines
 * themselves,  and unit labels on the axes.
 */
static void DrawGridAndAxis(LocalWin *wi)
{
	int     expX,
	...
	if (expY != 0) {
		...
		Xspot = wi->dev_info.bdr_pad + ((strlen(YUnitText) + 5) * wi->dev_info.axis_width);

		Yspot = wi->dev_info.bdr_pad * 2 + wi->dev_info.title_height + wi->dev_info.axis_height / 2;

		wi->dev_info.xg_text(wi->dev_info.user_state, Xspot, Yspot, final, T_RIGHT, T_AXIS);

		(void)sprintf(power, "%d", expY);

		wi->dev_info.xg_text(wi->dev_info.user_state, Xspot, Yspot, power, T_LOWERLEFT, T_AXIS);
	}
	else {
		Yspot = wi->dev_info.bdr_pad * 2 + wi->dev_info.title_height;

		wi->dev_info.xg_text(wi->dev_info.user_state, wi->dev_info.bdr_pad, Yspot, YUnitText, T_UPPERLEFT, T_AXIS);
	}
	...
	if (expX != 0) {
		...
		wi->dev_info.xg_text(wi->dev_info.user_state, startX, wi->XOppY, power, T_LOWERLEFT, T_AXIS);

		(void)strcpy(final, XUnitText);
		(void)strcat(final, " x 10");
		wi->dev_info.xg_text(wi->dev_info.user_state, startX, wi->XOppY, final, T_RIGHT, T_AXIS);
	}
	else {
		wi->dev_info.xg_text(wi->dev_info.user_state, startX, wi->XOppY, XUnitText, T_RIGHT, T_AXIS);
	}
	...
	for (Yindex = Ystart; Yindex < wi->UsrOppY; Yindex = stepGrid()) {
		...
		wi->dev_info.xg_text(wi->dev_info.user_state, wi->dev_info.bdr_pad + (7 * wi->dev_info.axis_width), Yspot, value, T_RIGHT, T_AXIS);
	}
	...
	for (Xindex = Xstart; Xindex < wi->UsrOppX; Xindex = stepGrid()) {
		...
		wi->dev_info.xg_text(wi->dev_info.user_state, Xspot, wi->dev_info.area_h - wi->dev_info.bdr_pad, value, T_BOTTOM, T_AXIS);
	}
	...
	for (Yindex = Ystart; Yindex < wi->UsrOppY; Yindex = stepGrid()) {
		...
		if ((ABS(Yindex) < ZERO_THRES) && !logYFlag) {
			wi->dev_info.xg_seg(wi->dev_info.user_state, 1, segs, PM_INT("ZeroWidth"), L_ZERO, 0, 0);

			if (tickFlag) {
				wi->dev_info.xg_seg(wi->dev_info.user_state, 1, &(segs[1]), PM_INT("ZeroWidth"), L_ZERO, 0, 0);
			}
		}
		else {
			wi->dev_info.xg_seg(wi->dev_info.user_state, 1, segs, PM_INT("GridSize"), L_AXIS, 0, 0);
			if (tickFlag) {
				wi->dev_info.xg_seg(wi->dev_info.user_state, 1, &(segs[1]), PM_INT("GridSize"), L_AXIS, 0, 0);
			}
		}
	}
	...
	if (PM_BOOL("BoundBox")) {
		...
		wi->dev_info.xg_seg(wi->dev_info.user_state, 4, bb, PM_INT("GridSize"), L_AXIS, 0, 0);
	}
}
...
/*
 * This routine rounds up the given positive number such that it is some power
 * of ten times either 1, 2, or 5.  It is used to find increments for grid
 * lines.
 */
 ...
 */
static void WriteValue(char *str, char *fmt, double val, int expv, int logFlag)
{
	int     idx;
...
/*
 * This routine draws the data sets themselves using the macros for translating
 * coordinates.
 */
		...
		while (thisList) {
			...
			for (subindex = 0; subindex < thisList->numPoints - 1; subindex++) {
				...
				if (markFlag && mark_inside) {
					if (pixelMarks) {
						if (bigPixel) {
							wi->dev_info.xg_dot(wi->dev_info.user_state, Xsegs[1][X_idx - 1].x1, Xsegs[1][X_idx - 1].y1, P_DOT, 0, idx % MAXATTR);
						}
						else {
							wi->dev_info.xg_dot(wi->dev_info.user_state, Xsegs[1][X_idx - 1].x1, Xsegs[1][X_idx - 1].y1, P_PIXEL, 0, PIXVALUE(idx));
						}
					}
					else {
						/* Distinctive markers */
						wi->dev_info.xg_dot(wi->dev_info.user_state, Xsegs[1][X_idx - 1].x1, Xsegs[1][X_idx - 1].y1, P_MARK, MARKSTYLE(idx), PIXVALUE(idx));
					}
				}
				...
				if (PM_BOOL("BarGraph")) {
					...
					line.x1 = line.x2 = Xsegs[1][X_idx - 1].x1 + (int) ((PM_DBL("BarOffset") * idx / wi->XUnitsPerPixel) + 0.5);
					...
					wi->dev_info.xg_seg(wi->dev_info.user_state, 1, &line, barPixels, L_VAR, LINESTYLE(idx), PIXVALUE(idx));
				}
			}
			/* Handle last marker */
			if (markFlag && (thisList->numPoints > 0)) {
				C_CODE(thisList->xvec[thisList->numPoints - 1], thisList->yvec[thisList->numPoints - 1], mark_inside);

				if (mark_inside == 0) {
					if (pixelMarks) {
						if (bigPixel) {
							wi->dev_info.xg_dot(wi->dev_info.user_state, Xsegs[1][X_idx - 1].x2, Xsegs[1][X_idx - 1].y2, P_DOT, 0, idx % MAXATTR);
						}
						else {
							wi->dev_info.xg_dot(wi->dev_info.user_state, Xsegs[1][X_idx - 1].x2, Xsegs[1][X_idx - 1].y2, P_PIXEL, 0, PIXVALUE(idx));
						}
					}
					else {
						/* Distinctive markers */
						wi->dev_info.xg_dot(wi->dev_info.user_state, Xsegs[1][X_idx - 1].x2, Xsegs[1][X_idx - 1].y2, P_MARK, MARKSTYLE(idx), PIXVALUE(idx));
					}
				}
			}
			/* Handle last bar */
			if ((thisList->numPoints > 0) && PM_BOOL("BarGraph")) {
				...
				baseSpot = SCREENY(wi, PM_DBL("BarBase"));
				line.x1 = line.x2 = Xsegs[1][X_idx - 1].x2 + (int) ((PM_DBL("BarOffset") * idx / wi->XUnitsPerPixel) + 0.5);
			...
			if ((thisList->numPoints > 0) && (!noLines) && (X_idx > 0)) {
				...
				while (X_idx > wi->dev_info.max_segs) {
					wi->dev_info.xg_seg(wi->dev_info.user_state, wi->dev_info.max_segs, ptr, lineWidth, L_VAR, 16, (int)(1));
				...
				wi->dev_info.xg_seg(wi->dev_info.user_state, X_idx, ptr, lineWidth, L_VAR, 16,(int)(1));
			}
			/* Next subset */
 ...
 * coordinates.
 */
static void DrawData(LocalWin *wi)
{
	double  sx1,
			sy1,
			sx2,
		...
		while (thisList) {
			...
			if (!PM_BOOL("Animate")) {
				if (thisList->numPoints > 0 && (!noLines) && (X_idx > 0)) {
					...
					while (X_idx > wi->dev_info.max_segs) {
						wi->dev_info.xg_seg(wi->dev_info.user_state, wi->dev_info.max_segs, ptr, lineWidth, L_VAR, LINESTYLE(idx), PIXVALUE(idx));
						ptr += wi->dev_info.max_segs;
						X_idx -= wi->dev_info.max_segs;
					}

					wi->dev_info.xg_seg(wi->dev_info.user_state, X_idx, ptr, lineWidth, L_VAR, LINESTYLE(idx), PIXVALUE(idx));
				}
			} else {
				StoreIDX = X_idx;
				...
				if (thisList->numPoints > 0 && (!noLines) && (X_idx > 0)) {
					...
					while (X_idx > wi->dev_info.max_segs) {
						wi->dev_info.xg_seg(wi->dev_info.user_state, wi->dev_info.max_segs, ptr, lineWidth, L_VAR, LINESTYLE(1), PIXVALUE(2));
						ptr += wi->dev_info.max_segs;
						X_idx -= wi->dev_info.max_segs;
					}

					wi->dev_info.xg_seg(wi->dev_info.user_state, X_idx, ptr, lineWidth, L_VAR, LINESTYLE(1), PIXVALUE(2));
				}
 ...
 * are drawn.
 */
static void DrawLegend(LocalWin *wi)
{
	int     idx,
			spot,
			lineLen,
	...
	for (idx = 0; idx < MAXSETS; idx++) {
		if ((PlotData[idx].list) && (spot + wi->dev_info.axis_height + 2 < wi->XOppY)) {
			/* Meets the criteria */
			...
			wi->dev_info.xg_text(wi->dev_info.user_state, wi->XOppX + wi->dev_info.bdr_pad, spot + 2, PlotData[idx].setName, T_UPPERLEFT, T_AXIS);
			spot += incr;
		}
	}
	...
	for (idx = 0; idx < MAXSETS; idx++) {
		if ((PlotData[idx].list) && (spot + wi->dev_info.axis_height + 2 < wi->XOppY)) {
			...
			if (PM_BOOL("BarGraph")) {
				wi->dev_info.xg_seg(wi->dev_info.user_state, 1, &leg_line, incr / 4, L_VAR, LINESTYLE(idx), PIXVALUE(idx));
			}

			if (!PM_BOOL("NoLines")) {
				wi->dev_info.xg_seg(wi->dev_info.user_state, 1, &leg_line, 1, L_VAR, LINESTYLE(idx), PIXVALUE(idx));
			}

			if (markFlag && !pixelMarks) {
				wi->dev_info.xg_dot(wi->dev_info.user_state, leg_line.x1, leg_line.y1, P_MARK, MARKSTYLE(idx), PIXVALUE(idx));
			}
			spot += incr;
		}
	}
...
/*
 * Determines the values of the old boolean flags based on the new values in
 * the parameters database.
 */
static void set_mark_flags(int *markFlag, int *pixelMarks, int *bigPixel, int *colorMark)
{
	*markFlag = 0;
	*pixelMarks = 0;
	*colorMark = 0;
