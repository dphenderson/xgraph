...
/*
 * The basic coordinate system is points (roughly 1/72 inch).  However,  most
 * laser printers can do much better than that.  We invent a coordinate system
 * based on VDPI dots per inch.  This goes along the long side of the page.
 * The long side of the page is LDIM inches in length,  the short side SDIM
 * inches in length.  We we call this unit a `dev'.  We map `width' and
 * `height' into devs.
 */
int psInit(FILE *psFile, int width, int height, char *tf, char *af, double ts, double as, int flags, xgOut *outInfo, char errmsg[ERRBUFSIZE])
{
	struct userInfo *ui;
	double  font_size;
...
/*
 * This routine figures out how transform the basic postscript transformation
 * into one suitable for direct use by the drawing primitives.  Two variables
 * X-CENTER-PLOT and Y-CENTER-PLOT determine whether the plot is centered on
 * the page.  If `flags' has D_DOCU set,  then the plot will not be rotated or
 * centered and a bounding box will be displayed.
 */
static void psScale(FILE *psFile, int width, int height, int flags)
{
	...
	if (flags & D_DOCU) {
		OUT(psFile, "%%%%BoundingBox: %ld %ld %ld %ld\n", (long)0, (long)0, (long)(((double) width) / (MICRONS_PER_INCH * INCHES_PER_POINT) + 0.5),
				(long)(((double) height) / (MICRONS_PER_INCH * INCHES_PER_POINT) + 0.5));
		psHeader(psFile, 1);
		...
		/* First: rotation.  If the width is greater than the short dimension, do the rotation. */
 ...
 * and style.
 */
static void psText(char *state, int x, int y, char *text, int just, int style)
{
 ...
 */
static void psSeg(char *state, int ns, XSegment *seglist, int width, int style, int lappr, int color)
{
	struct userInfo *ui = (struct userInfo *)state;
	int     newwidth = 0,
			i;
 ...
 */
static void psDot(char *state, int x, int y, int style, int type, int color)
{
	struct userInfo *ui = (struct userInfo *) state;
...
static void psEnd(char *userState)
{
