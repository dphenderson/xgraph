...
/*
 * This is the handler function for the text widget for specifing the file or
 * device name.  It supports simple line editing operations.
 */
...
/*
 * This is the handler function for when the `Ok' button is hit.  It sets the
 * button,  does the hardcopy output, and turns the button off.  It returns a
 * status which deactivates the dialog.
 */
static xtb_hret ok_fun(Window win, int bval, xtb_data info)
{
			...
			if (sscanf(dim_spec, "%lf", &centimeters) == 1) {
				...
				if (sscanf(tsizstr, "%lf", &tsize) == 1) {
					...
					if (sscanf(asizstr, "%lf", &asize) == 1) {
						...
						do_hardcopy(real_info->prog, real_info->cookie, hard_devices[val].dev_init, dev_spec, file_or_dev, centimeters, tfam, tsize, afam, asize, doc_p);
						rtn = XTB_STOP;
					}
					else {
						/* Bad axis size */
...
/*
 * This is the handler function for the cancel button.  It turns itself on and
 * off and then exits with a status which kills the dialog.
 */
static xtb_hret can_fun(Window win, int val, xtb_data info)
{
	(void) xtb_bt_set(win, 1, (xtb_data)0, 0);
	(void) xtb_bt_set(win, 0, (xtb_data)0, 0);
	return XTB_STOP;
...
/*
 * This is the handler function for the document button.  It toggles it's
 * state.
 */
static xtb_hret docu_fun(Window win, int val, xtb_data info)
{
	int     state;
...
/*
 * This routine swaps the device specific information in the dialog based on
 * what device is selected.  The information passed is the information for the
 * whole dialog.
 */
static xtb_hret dev_fun(Window win, int old, int new, xtb_data info)
{
	...
	if ((new >= 0) && (new < hard_count)) {
		if (fod_spot == 1) {
			...
			xtb_ti_set(data->fodspec, hard_devices[new].dev_printer, (xtb_data)0);
		}
		else {
		...
		xtb_ti_set(data->tf_family, hard_devices[new].dev_title_font, (xtb_data)0);

		(void)sprintf(text, "%lg", hard_devices[new].dev_title_size);

		xtb_ti_set(data->tf_size, text, (xtb_data)0);
		xtb_ti_set(data->af_family, hard_devices[new].dev_axis_font, (xtb_data)0);
...
/*
 * This routine swaps the default file or device names based on the state of
 * the file or device buttons.  The information passed is the information for
 * the whole dialog.
 */
static xtb_hret fd_fun(Window win, int old, int new, xtb_data info)
{
	...
	if ((which_one >= 0) && (which_one < hard_count)) {
		...
		if (new == 0) {
			/* Restore into device */
			xtb_ti_set(data->fodspec, hard_devices[which_one].dev_printer, (xtb_data *)0);
		}
		else if (new == 1) {
			xtb_ti_set(data->fodspec, hard_devices[which_one].dev_file, (xtb_data *)0);
		}
	}
	return XTB_HANDLED;
}
...
/*
 * This routine constructs a new dialog for asking the user about hardcopy
 * devices.  The dialog and its size is returned in `frame'.  The window of the
 * `ok' button is returned in `btnwin'.  This can be used to reset some of the
 * button state to reuse the dialog.
 */
static void make_dialog(Window win, Window spawned, char *prog, xtb_data cookie, xtb_frame *okbtn, xtb_frame *frame)
{
	Window  overall;
	xtb_frame AF(LAST_F);
	xtb_fmt *def,
	...
	wamask = CWBackPixel | CWBorderPixel | CWOverrideRedirect | CWSaveUnder | CWColormap;
	...
	overall = XCreateWindow(disp, win, 0, 0, 1, 1, D_BRDR, depth, InputOutput, vis, wamask, &wattr);
	...
	xtb_br_new(overall, hard_count, names, found, dev_fun, (xtb_data)info, &AF(ODEVROW_F));
	...
	xtb_br_new(overall, 2, fodstrs, found, fd_fun, (xtb_data)info, &AF(DISPROW_F));
	 ...
	 * wimpy.
	 */
	cntrl = xtb_vert(XTB_LEFT, D_VPAD, D_INT, xtb_hort(XTB_CENTER, D_HPAD, D_INT, xtb_w(&AF(ODEVLBL_F)), xtb_w(&AF(ODEVROW_F)), NE),
			xtb_hort(XTB_CENTER, D_HPAD, D_INT, xtb_w(&AF(DISPLBL_F)), xtb_w(&AF(DISPROW_F)), NE),
			xtb_hort(XTB_CENTER, D_HPAD, D_INT, xtb_w(&AF(FDLBL_F)), xtb_w(&AF(FDINP_F)), NE), NE);

	mindim = xtb_vert(XTB_LEFT, D_VPAD, D_INT, xtb_hort(XTB_CENTER, D_HPAD, D_INT, xtb_w(&AF(MDIMLBL_F)), xtb_w(&AF(MDIMI_F)), NE), NE);

	tfarea = xtb_vert(XTB_LEFT, D_VPAD, D_INT, xtb_hort(XTB_CENTER, D_HPAD, D_INT, xtb_w(&AF(TFFAMLBL_F)), xtb_w(&AF(TFFAM_F)),
				xtb_w(&AF(TFSIZLBL_F)), xtb_w(&AF(TFSIZ_F)), NE), NE);

	afarea = xtb_vert(XTB_LEFT, D_VPAD, D_INT, xtb_hort(XTB_CENTER, D_HPAD, D_INT, xtb_w(&AF(AFFAMLBL_F)), xtb_w(&AF(AFFAM_F)),
				xtb_w(&AF(AFSIZLBL_F)), xtb_w(&AF(AFSIZ_F)), NE), NE);

	def = xtb_fmt_do(xtb_vert(XTB_CENTER, D_VPAD, D_INT, xtb_w(&AF(TITLE_F)), cntrl, xtb_w(&AF(BAR_F)), xtb_w(&AF(OPTLBL_F)), mindim,
				xtb_w(&AF(DOCU_F)), xtb_w(&AF(TFLBL_F)), tfarea, xtb_w(&AF(AFLBL_F)), afarea, xtb_hort(XTB_CENTER, D_HPAD, D_INT,
					xtb_w(&AF(OK_F)), xtb_w(&AF(CAN_F)), NE), NE), &frame->width, &frame->height);
...
/*
 * Makes a shadow window for a pop-up window of the specified size.  Needs hint
 * work as well.  Try no background window.
 */
...
/*
 * Asks the user about hardcopy devices.  A table of hardcopy device names and
 * function pointers to their initialization functions is assumed to be in the
 * global `hard_devices' and `hard_count'.  Returns a non-zero status if the
 * dialog was sucessfully posted.  Calls do_hardcopy in xgraph to actually
 * output information.
 */
void ho_dialog(Window parent, char *prog, xtb_data cookie)
{
	...
	if (!overall.win) {
		make_dialog(RootWindow(disp, screen), parent, prog, cookie, &okbtn, &overall);
#ifdef SHADOW
	...
	XTranslateCoordinates(disp, parent, RootWindow(disp, screen), 0, 0, &winInfo.x, &winInfo.y, &dummy);
	XMoveWindow(disp, overall.win, (int)(winInfo.x + winInfo.width / 2 - overall.width / 2), (int)(winInfo.y + winInfo.height / 2 - overall.height / 2));
...
#ifdef SHADOW
	XMoveWindow(disp, shadow, winInfo.x + winInfo.width / 2 - d_w / 2 + SH_W, winInfo.y + winInfo.height / 2 - d_h / 2 + SH_H);
	hints.flags = PPosition;
 ...
 */
static xtb_hret err_func(Window win, int bval, xtb_data info)
{
    (void)xtb_bt_set(win, 1, (xtb_data)0, 0);
    (void)xtb_bt_set(win, 0, (xtb_data)0, 0);
    return XTB_STOP;
 ...
 */
static void make_msg_box(char *text, char *title, xtb_frame *frame)
{
	XSizeHints hints;
	...
	frame->win = XCreateWindow(disp, RootWindow(disp, screen), 0, 0, 1, 1, D_BRDR, depth, InputOutput, vis, wamask, &wattr);
	...
	while (xgraph_getline(&lineptr, line)) {
		if (new_info->num_lines >= new_info->alloc_lines) {
			...
			new_info->lines = (Window *)Realloc((char *)new_info->lines, (unsigned)(new_info->alloc_lines * sizeof(Window)));
			memset(((char*)new_info->lines) + old_alloc_lines_size, 0, old_alloc_lines_size);
		}
...
/*
 * This posts a dialog that contains lines of text and a continue button.  The
 * text may be multiple lines.  The dialog is remade each time.
 */
void msg_box(char *title, char *text)
{
#ifdef SHADOW
	Window  shadow;
#endif
	...
	XMoveWindow(disp, text_frame.win, (int) (info.width / 2 - text_frame.width / 2), (int) (info.height / 2 - text_frame.height / 2));
...
#ifdef SHADOW
	XMoveWindow(disp, shadow, info.width / 2 - w / 2 + SH_W, info.height / 2 - h / 2 + SH_H);
...
/*
 * Returns next line from tptr.  Assumes `lptr' is large enough to accept the
 * line.
 */
int xgraph_getline(char **tptr, char *lptr)
{
