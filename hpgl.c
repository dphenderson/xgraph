...
int hpglInit(FILE *stream, int width, int height, char *title_family, double title_size, char *axis_family,
		double axis_size, int flags, xgOut *outInfo, char errmsg[ERRBUFSIZE])
{
	mydata *myInfo;
	...
	*outInfo = hpglInfo;
	...
	fprintf(myInfo->plotterFile, "IW%d,%d,%d,%d;\n", MAPX(myInfo, 0), MAPY(myInfo, myInfo->clipmaxY - myInfo->clipminY),
			MAPX(myInfo, myInfo->clipmaxX - myInfo->clipminX), MAPY(myInfo, 0));
...
static int lineselect[8] = {LINE1, LINE2, LINE3, LINE4, LINE5, LINE6, LINE7, LINE8};
...
static void hpglSeg(mydata *userState, int ns, XSegment *segs, int width, int style, int lappr, int color)
{
	...
	for (i = 0; i < ns; i++) {
		if (!i || ((segs[i].x1 != segs[i - 1].x2) || (segs[i].y1 != segs[i - 1].y2))) {
			/* MOVE */
			fprintf(userState->plotterFile, "PU;PA%d,%d;\n", MAPX(userState, segs[i].x1), MAPY(userState, segs[i].y1));
		}
		/* DRAW */
		if (width <= 1) {
			fprintf(userState->plotterFile, "PD;PA%d,%d;\n", MAPX(userState, segs[i].x2), MAPY(userState, segs[i].y2));
		}
			...
			frac = (width / 2) / sqrt((double)((segs[i].x1 - segs[i].x2) * (segs[i].x1 - segs[i].x2)) + ((segs[i].y1 - segs[i].y2) * (segs[i].y1 - segs[i].y2)));
			...
			fprintf(userState->plotterFile, "PU;PA%d,%d;", MAPX(userState, llx), MAPY(userState, lly));
			fprintf(userState->plotterFile, "PM0;");
			fprintf(userState->plotterFile, "PD,PA%d,%D;PA%d,%D;PA%d,%d;\n", MAPX(userState, lrx), MAPY(userState, lry), MAPX(userState, urx), MAPY(userState, ury), MAPX(userState, ulx), MAPY(userState, uly));
...
static char *markselect[8] = {MARK1, MARK2, MARK3, MARK4, MARK5, MARK6, MARK7, MARK8};
