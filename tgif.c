...
int tgifInit(FILE *strm, int width, int height, char *title_family, double title_size, char *axis_family, double axis_size, int flags, xgOut *out_info,
...
/*
 * Unfortunately, tgif really can't display text with a justification.  This is
 * a horrible hack to try to get around the problem.  It tries to compute a
 ...
 * hack...
 */
static void tgif_just(int *x, int *y, int just, int size, int len)
{
	int     t_width,
			t_height;
 ...
 */
void tgifText(char *user_state, int x, int y, char *text, int just, int style)
{
	char   *font;
	int     size;
	...
	/* font ok too */
	font = style == T_AXIS ? tgif->axis_font : tgif->title_font;

	/* ok 0, 1 as in tgif */
	size = style == T_AXIS ? tgif->axis_size : tgif->title_size;
	...
	fprintf(tgif->strm, "text('%s',%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,[\n\t", COLOR, x, y, 0, style, size, 1, 0, 0, 1, 0, 0, 0, 0, 18, 4, 0, 0);
	fprintf(tgif->strm, "\"%s\"]).\n", text);
}
...
void tgifSeg(char *user_state, int ns, XSegment *seglist, int width, int style, int lappr, int color)
{
	Info   *tgif = (Info *) user_state;
	int     i,
			j,
	...
	for (i = 0; i < ns; i++) {
		fprintf(tgif->strm, "poly('%s',2,[%d,%d,%d,%d],", COLOR, seglist[i].x1, seglist[i].y1, seglist[i].x2, seglist[i].y2);
		fprintf(tgif->strm, "%d,%d,%d,%d,%d,%d,%d,[\n]).\n", 0, width, style_list[lappr], 0, 0, style_list[lappr], 0);
	}
}
 ...
 */
void tgifDot(char *user_state, int x, int y, int style, int type, int color)
{
}
