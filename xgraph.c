...
Window NewWindow(char *progname, double lowX, double lowY, double upX, double upY, double asp, int DO);
...
NewDataSet PlotData[MAXSETS], DataD1[MAXSETS], DataD2[MAXSETS];
...
/*
 * This sets up the hard-wired defaults and reads the X defaults.  The command
 * line format is: xgraph [host:display].
 */
int main(int argc, char *argv[])
{
	...
	if (PM_BOOL("Animate")) {
		param_set("TitleText",STR,"Animated X Graph");
	}
	...
	if (PM_INT("Output Device") == D_XWINDOWS) {
		...
		primary = NewWindow(Prog_Name, PM_DBL("XLowLimit"), PM_DBL("YLowLimit"), PM_DBL("XHighLimit"), PM_DBL("YHighLimit"), asp, 0);
		if (!primary) {
		...
		while (Num_Windows > 0) {
			...
			if (XFindContext(theEvent.xany.display, theEvent.xany.window, win_context, (caddr_t *) & win_info)) {
				/* Nothing found */
				continue;
			}
			...
			switch (theEvent.type) {
				case Expose:
					...
					break;
				case KeyPress:
					nbytes = XLookupString(&theEvent.xkey, keys, MAXKEYS, (KeySym *) 0, (XComposeStatus *) 0);
				...
				case ButtonPress:
					...
					Num_Windows += HandleZoom(Prog_Name, &theEvent.xbutton, win_info, zoomCursor);
					break;
				default:
		...
		primary = NewWindow(Prog_Name, PM_DBL("XLowLimit"), PM_DBL("YLowLimit"), PM_DBL("XHighLimit"), PM_DBL("YHighLimit"), 1.0, 0);

		do_hardcopy(Prog_Name, primary, hard_devices[Device].dev_init, dflag ? hard_devices[Device].dev_spec : 0, PM_STR("FileOrDev"), (double) 19,
				hard_devices[Device].dev_title_font, hard_devices[Device].dev_title_size, hard_devices[Device].dev_axis_font,
				hard_devices[Device].dev_axis_size, PM_BOOL("Document") * D_DOCU);
	}
	return 0;
}
...
/*
 * Looks up `param_name' in the parameters database.  If found, the color is
 * examined and judged to be either black or white based upon its red, green,
 * and blue intensities.  The sense of the color is then reversed and reset to
 * its opposite.
 */
static void ReversePix(char *param_name)
{
	params  val;

	if (param_get(param_name, &val)) {
		if ((val.pixv.value.red < BLACK_THRES) && (val.pixv.value.green < BLACK_THRES) && (val.pixv.value.blue < BLACK_THRES)) {
			/* Color is black */
...
/*
 * This routine attempts to implement reverse video.  It steps through all of
 * the important colors in the parameters database and makes black white (and
 * vice versa).
 */
static void ReverseIt()
{
...
/*
 * Traverses through all of the data applying certain options to the data and
 * computing the overall bounding box.  The flags are: LOG_X	Take the log of
 * the X axis LOG_Y	Take the log of the Y axis STK	Stack coordinates.  FITX
 * Fit x-coordinates from zero to one FITY	Fit y-coordinates from zero to one
 */
static void Traverse(int flags)
{
	int     i,
			j;
	PointList *spot;
	...
	if (flags & STK) {
		for (i = 1; i < MAXSETS; i++) {
			for (spot = PlotData[i].list, pspot = PlotData[i - 1].list; spot && pspot; spot = spot->next, pspot = pspot->next) {
...
/*
 * This routine is called when the `Close' button is pressed in an xgraph
 * window.  It causes the window to go away.
 */
...
/*
 * This routine is called when the hardcopy button is pressed in an xgraph
 * window.  It causes the output dialog to be posted.
 */
...
/*
 * This routine added, Paul Walker, to rewind the animation and start it over.
 * The only even moderatly tricky part is erasing the last item, which still
 * lives, and redrawing the axis.  I do it by just copying the "Expose"
 * information from the main routine.
 */
static xtb_hret rew_func(Window win, int bval, xtb_data info)
{
	Window  the_win = (Window) info;
	LocalWin *win_info;
 ...
 * information from the main routine.
 */
static xtb_hret der_func(Window win, int bval, xtb_data info)
{
	Window  the_win = (Window) info;
	Window new_win;
	LocalWin *win_info;
...
static xtb_hret rpl_func(Window win, int bval, xtb_data info)
{
...
/*
 * Creates and maps a new window.  This includes allocating its local structure
 * and associating it with the XId for the window.  The aspect ratio is
 * specified as the ratio of width over height.
 */
Window NewWindow(char *progname, double lowX, double lowY, double upX, double upY, double asp, int DO)
{
	Window  new_window;
	...
	if (PM_INT("Output Device") == D_XWINDOWS) {
		...
		geo_mask = XParseGeometry(PM_STR("Geometry"), &sizehints.x, &sizehints.y, (unsigned int *) &sizehints.width, (unsigned int *) &sizehints.height);
		...
		new_window = XCreateWindow(disp, RootWindow(disp, screen), sizehints.x, sizehints.y, (unsigned int)sizehints.width, (unsigned int)sizehints.height,
				(unsigned int)PM_INT("BorderSize"), depth, InputOutput, vis, wamask, &wattr);
		...
		if (new_window) {
			...
			if (!PM_BOOL("NoButton")) {
				...
				XMoveWindow(disp, new_info->hardcopy, (int) (BTNPAD + cl_frame.width + BTNINTER), BTNPAD);

				xtb_bt_new(new_window, "About", abt_func, (xtb_data) new_window, &ab_frame);
				new_info->about = ab_frame.win;
				XMoveWindow(disp, new_info->about, (int) (BTNPAD + cl_frame.width + BTNINTER + hd_frame.width + BTNINTER), BTNPAD);
#ifdef DO_DER
				/* These buttons added PW */
				xtb_bt_new(new_window, "Anim", rew_func, (xtb_data) new_window, &rw_frame);
				new_info->rewind = rw_frame.win;
				XMoveWindow(disp, new_info->rewind, (int) (BTNPAD + cl_frame.width + BTNINTER + hd_frame.width + BTNINTER + ab_frame.width + BTNINTER), BTNPAD);

				xtb_bt_new(new_window, "Replot", rpl_func, (xtb_data) new_window, &rp_frame);
				new_info->replot = rp_frame.win;
				XMoveWindow(disp, new_info->replot, (int) (BTNPAD + cl_frame.width + BTNINTER + hd_frame.width + BTNINTER + ab_frame.width + BTNINTER + rw_frame.width + BTNINTER), BTNPAD);

				xtb_bt_new(new_window, "Deriv", der_func, (xtb_data) new_window, &dx_frame);
				new_info->deriv = dx_frame.win;
				XMoveWindow(disp, new_info->deriv, (int) (BTNPAD + cl_frame.width + BTNINTER + hd_frame.width + BTNINTER + ab_frame.width + BTNINTER + rw_frame.width + BTNINTER + rp_frame.width + BTNINTER), BTNPAD); 
#endif /* DO_DER */

				new_info->flags = 0;
			}

			XSelectInput(disp, new_window, ExposureMask | KeyPressMask | ButtonPressMask);
...
/*
 * This routine actually deletes the specified window and decrements the window
 * count.
 */
void DelWindow(Window win, LocalWin *win_info)
{
...
/*
 * This routine posts a dialog asking about the hardcopy options desired.  If
 * the user hits `OK',  the hard copy is performed.
 */
void PrintWindow(Window win, LocalWin *win_info)
{
...
int HandleZoom(char *progname, XButtonPressedEvent *evt, LocalWin *wi, Cursor  cur)
{
	...
	if (XGrabPointer(disp, win, True, (unsigned int)(ButtonPressMask|ButtonReleaseMask|PointerMotionMask|PointerMotionHintMask), GrabModeAsync, GrabModeAsync, win, cur, CurrentTime) != GrabSuccess) {
		XBell(disp, 0);
		return 0;
	}
	...
	startX = evt->x;
	startY = evt->y;
	XQueryPointer(disp, win, &root_rtn, &child_rtn, &root_x, &root_y, &curX, &curY, &mask_rtn);
	...
	while (!stopFlag) {
		...
		switch (theEvent.xany.type) {
			case MotionNotify:
				XQueryPointer(disp, win, &root_rtn, &child_rtn, &root_x, &root_y, &newX, &newY, &mask_rtn);
			...
			case ButtonRelease:
				...
				if ((startX - curX != 0) && (startY - curY != 0)) {
					...
					new_win = NewWindow(progname, loX, loY, hiX, hiY, asp, wi->DOrder);
...
/*
 * This routine resets the function pointers to those specified by `init_fun'
 * and causes a screen redisplay.  If `dev_spec' is non-zero,  it will be
 * considered a sprintf string with one %s which will be filled in with
 * `file_or_dev' and fed to popen(3) to obtain a stream.  Otherwise,
 * `file_or_dev' is considered to be a file and is opened for writing.  The
 * resulting stream is fed to the initialization routine for the device.
 */
void do_hardcopy(char *prog, char *info, int (*init_fun) (), char *dev_spec, char *file_or_dev, double maxdim, char *ti_fam, char *ax_fam, double ti_size, double ax_size, int doc_p)
{
	...
	if ((*init_fun) (out_stream, final_w, final_h, ti_fam, ti_size, ax_fam, ax_size, flags, &(thisWin.dev_info), ierr)) {
		DrawWindow(&thisWin);
...
/*
 * This routine expands out a file name passed in `in' and places the expanded
 * version in `out'.  It returns `out'.
 */
static char *tildeExpand(char *out, char *in)
{
	char    username[50],
			*userPntr;
...
static int XErrHandler(Display *disp_ptr, XErrorEvent *evt)
{
	...
	XGetErrorDatabaseText(disp_ptr, mtype, "MajorCode", "Request Major code %d", mesg, ERR_MSG_SIZE);
	(void)fprintf(stderr, mesg, evt->request_code);

	(void)sprintf(number, "%d", evt->request_code);

	XGetErrorDatabaseText(disp_ptr, "XRequest", number, "", err_buf, ERR_MSG_SIZE);
