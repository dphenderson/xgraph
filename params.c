...
/*
 * Initializes parameter package.  The display and colormap arguments are used
 * to resolve font and pixel values.
 */
...
/*
 * Sets the parameter with the given name to have the type `type' and the text
 * value `value'.  This will be evaluated to its full form the first time it is
 * referenced using param_get().  If it is already filled, the old value will
 * be reclaimed.
 */
void param_set(char *name, param_types type, char *val)
{
	param_full *entry;
...
/*
 * This routine sets the value of an existing parameter to a new value.  The
 * type of the parameter remains the same.  Changes in type should be done by
 * using param_set() directly.
 */
void param_reset(char *name, char *val)
{
	param_full *entry;
...
/*
 * Retrieves a value from the parameter table.  The value is placed in `val'.
 * If successful, the routine will return `val'.  Otherwise, it will return
 * zero.
 */
params *param_get(char *name, params *val)
{
	param_full *entry;
...
static void free_resource(params *val)
{
	switch (val->type) {
		...
		case DBL:
			...
			break;
		case PIXEL:
			if ((val->pixv.value.pixel != WhitePixel(param_disp, param_scrn)) && (val->pixv.value.pixel != BlackPixel(param_disp, param_scrn))) {
...
/*
 * Allocates and returns an appropriate parameter structure by translating
 * `form' into its native type as given by `type'.  If it can't translate the
 * given form, it will fall back onto the default.
 */
static params *resolve_entry(char *name, param_types type, char *form)
{
	static char paramstr[] = "Parameter %s: can't translate `%s' into a %s (defaulting to `%s')\n";
	...
	switch (type) {
		...
		case STYLE:
			if (!do_style(form, &result->stylev)) {
				(void)fprintf(stderr, paramstr, name, form, "line style", DEF_STYLE);
				(void)do_style(DEF_STYLE, &result->stylev);
			}
			break;
		case BOOL:
			if (!do_bool(form, &result->boolv.value)) {
				(void)fprintf(stderr, paramstr, name, form, "boolean flag", DEF_BOOL);
...
/*
 * Translates `name' into a color and attempts to get the pixel for the color
 * using XAllocColor().
 */
static int do_color(char *name, XColor *color)
{
	int     result = 1;
...
/*
 * This routine translates a font name into a font structure.  The font name
 * can be in two forms.  The first form is <family>-<size>.  The family is a
 * family name (like helvetica) and the size is in points (like 12).  If the
 * font is not in this form, it is assumed to be a regular X font name
 * specification and is looked up using the standard means.
 */
static int do_font(char *name, XFontStruct **font_info)
{
		...
		if ((font_size = index(name_copy, '-'))) {
			*font_size = '\0';
			...
			if (font_size_value > 0) {
				...
				(void)sprintf(query_spec, ISO_FONT, font_family, font_size_value * 10);

				font_list = XListFonts(param_disp, query_spec, DEF_MAX_NAMES, &font_count);
...
/*
 * Translates a string representation of a dash specification into a form
 * suitable for use in XSetDashes().  Assumes `list' is a null terminated
 * string of ones and zeros.
 */
static int do_style(char *list, param_style *val)
{
	char   *i,
		   *spot,
		   last_char;
...
/*
 * Translates a string representation into a suitable binary value.  Can parse
 * all kinds of interesting boolean type words.
 */
static int do_bool(char *name, int *val)
{
	char  **term;
...
static enum st_retval dump_it(char *key, char *value, char *arg)
{
 ...
 */
int stricmp(register char *a, register char *b)
{
	register int value;
	...
	for (; ((*a | *b) && !(value = ((isupper(*a) ? *a - 'A' + 'a' : *a) - (isupper(*b) ? *b - 'A' + 'a' : *b)))); a++, b++) {
		/* Empty Body */ ;
	}
...
static int strihash(register char *string, int modulus)
{
	register int val = 0;
	register int c;
