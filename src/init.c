...
/* Default line styles */
static char *defStyle[MAXATTR] = {"1", "10", "11110000", "010111", "1110", "1111111100000000", "11001111", "0011000111"};

/* Default color names */
/*
 * static char *defColors[MAXATTR] = {"red", "SpringGreen", "blue", "yellow", "cyan", "sienna", "orange", "coral"};
 */

static char *defColors[MAXATTR] = {"red", "SpringGreen", "blue", "yellow", "purple", "orange", "hotpink", "cyan"};

/*
 * Initializes the data sets with default information.  Sets up original values
 * for parameters in parameters package.
 */
void InitSets(int o)
{
	int     idx;
...
static void argerror(char *err, char *val)
{
...
/*
 * This routine parses the argument list for xgraph.  There are too many to
 * mention here so I won't.  If `do_it' is non-zero, options are actually
 * changed.  If `do_it' is zero, the argument list is parsed but the options
 * aren't set.  The routine is called once to obtain the input files then again
 * after the data is read to set the options.
 */
int ParseArgs(int argc, char *argv[], int do_it)
{
	int     idx,
			set,
			dflag;
	...
	while (idx < argc) {
		if (argv[idx][0] == '-') {
			...
			if (sscanf(argv[idx], "-%d", &set) == 1) {
				...
				/* Doesn't make much sense to PW why this must be switched, but it must. */
				...
				if (strcmp(argv[idx], "-lx") == 0) {
					...
					if (idx + 1 >= argc) {
						argerror("missing coordinate(s)", argv[idx]);
					}
					...
					if ((hi = index(argv[idx + 1], ','))) {
						...
						argerror("limit coordinates not specified right", argv[idx]);
					}
